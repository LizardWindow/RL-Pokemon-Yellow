...
class YellowEnv(Env):
    ...
    def TrackerReset(self, isFirst = False):
        ...
        progressLog.write("\n")
        MEPercentage = 0
        if self.rewardTrackerMapExploration > 0:
            MEPercentage = (self.fRewardTrackerMapExploration / self.rewardTrackerMapExploration) *100
        progressLog.write("\nMap exploration reward total: " + str(self.fRewardTrackerMapExploration) + "/"+ str(self.rewardTrackerMapExploration)+ "/" +str(MEPercentage))
        WPPercentage = 0
        if self.rewardTrackerWorldProgression > 0:
            (self.fRewardTrackerWorldProgression / self.rewardTrackerWorldProgression) *100
        ...
        PCPercentage = 0
        if self.rewardTrackerPokemonCaught > 0:
            (self.fRewardTrackerPokemonCaught / self.rewardTrackerPokemonCaught) *100
        progressLog.write("\nPokemon caught reward total: " + str(self.fRewardTrackerPokemonCaught) + "/"+ str(self.rewardTrackerPokemonCaught) + "/"+str(PCPercentage))
        PLPercentage = 0
        if self.rewardTrackerPokemonLevels > 0:
            (self.fRewardTrackerPokemonLevels / self.rewardTrackerPokemonLevels) *100
        progressLog.write("\nPokemon levels reward total: " + str(self.fRewardTrackerPokemonLevels) + "/"+ str(self.rewardTrackerPokemonLevels) + "/"+str(PLPercentage))
        DDPercentage = 0
        if self.rewardTrackerDamageDealt > 0:
            DDPercentage = (self.fRewardTrackerDamageDealt / self.rewardTrackerDamageDealt) *100
        progressLog.write("\nDamage dealt reward total: " + str(self.fRewardTrackerDamageDealt)  + "/"+ str(self.rewardTrackerDamageDealt)  + "/"+str(DDPercentage))
        DRPercentage = 0
        if self.rewardTrackerDamageReceived > 0:
            DRPercentage =(self.fRewardTrackerDamageReceived / self.rewardTrackerDamageReceived) *100
        progressLog.write("\nDamage received reward total: " + str(self.fRewardTrackerDamageReceived) +"/"+ str(self.rewardTrackerDamageReceived) +"/"+str(DRPercentage))
        FRPercentage = 0
        if self.rewardTrackerFlagsReached > 0:
            FRPercentage = (self.fRewardTrackerFlagsReached / self.rewardTrackerFlagsReached) *100
